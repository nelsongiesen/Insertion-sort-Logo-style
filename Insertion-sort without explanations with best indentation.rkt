...
(define insert 
  (lambda (item sorted-ls) 
    (letrec ((helper (lambda (part-before part-after) 
                       (if (empty? part-after) 
      ...
      (helper empty-list sorted-ls)))) 
                       ...
                       ; We know we are done when the unsorted-part is empty.
(define insertion-sort 
  (lambda (input-ls) 
    (letrec ((helper (lambda (unsorted-list sorted-list) 
                       (if (empty? unsorted-list) 
                           sorted-list
                           (let ((item (first unsorted-list)))
                           (helper (without-first unsorted-list) 
                                   (insert item sorted-list)))))))
      (helper input-ls empty-list)))) 
